...
public class PermanentBoosts {
    ...
    public static void elfBoost() {
        for (Player p : RaceManager.elfMap) {
            p.addPotionEffect(new PotionEffect(PotionEffectType.SPEED, 20 * 5, 0, false));
            if (isInWater(p)) {
                p.addPotionEffect(new PotionEffect(PotionEffectType.REGENERATION, 20 * 5, 0, false));
    ...
    public static void enderbornBoost() {
        for (Player p : RaceManager.enderbornMap) {
            ...
            p.addPotionEffect(new PotionEffect(PotionEffectType.INVISIBILITY, 5 * 20, 0, false));
        }
    }
